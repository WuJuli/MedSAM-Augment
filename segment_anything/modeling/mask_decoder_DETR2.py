...
class MaskDecoder(nn.Module):
    def __init__(
        ...
        self.deformable_model = DeformableTransformer(d_model=32, nhead=4,
                                                      num_encoder_layers=1, num_decoder_layers=6, dim_feedforward=256,
                                                      dropout=0.,
    ...
    def forward(
        ...
        f1 = self.embedding_encoder(cloned_image_embeddings)
        f2 = self.compress_vit_feat(vit_features)
        # print(f1.shape, f2.shape, [1, 32, 256, 256])

        b, c, h, w = f1.shape
        detr_src = [f1, f2]
        detr_mask = [torch.zeros(b, h, w, dtype=torch.bool, device=interm_embeddings[0].device) for _ in range(2)]
        # Initialize with small random values near zero
        small_random_value = 1e-6  # You can adjust this value
        detr_pos_embed = [small_random_value * torch.randn(b, c, h, w, device=interm_embeddings[0].device) for _ in
                          range(2)]
        # print(detr_mask[0].device, detr_src[0].device, detr_pos_embed[0].device, 99)

        # print(detr_src[0].shape, len(detr_src))
        # print(detr_mask[0].shape, len(detr_mask))
        # print(detr_pos_embed[0].shape, len(detr_pos_embed))
        hq_features = self.deformable_model(detr_src, detr_mask, detr_pos_embed)
        # print(hq_features.device, 3)
        hq_features = hq_features.reshape(2, 256, 256, 32)
        hq_features = hq_features[0].unsqueeze(0).permute(0, 3, 1, 2)
        # print(hq_features.shape, 4)
        #
        # hq_features = f1 + f2
    ...
    def predict_masks(
        ...
        # print(src.device, pos_src.device, tokens.device, 7)
        # Run the transformer
