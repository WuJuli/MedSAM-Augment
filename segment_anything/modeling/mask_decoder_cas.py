...
class MaskDecoder(nn.Module):
    ...
    def predict_masks(
        ...
        mask_hf_token = torch.cat([self.mask_tokens.weight, self.hf_token.weight], dim=0)
        ...
        for i in range(stage):
            output_tokens = torch.cat([iou_token, mask_hf_token], dim=0)
            ...
            iou_token = hs[:, 0, :]
            mask_hf_token = hs[:, 1: (1 + self.num_mask_tokens), :].squeeze(0)

        iou_token_out = iou_token
        mask_tokens_out = mask_hf_token.unsqueeze(0)

        src_tf = src_tf.transpose(1, 2).view(b, c, h, w)

        upscaled_embedding_sam = self.output_upscaling(src_tf)
