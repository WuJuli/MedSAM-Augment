    ...
    def train(self, model, train_loader: Iterable, logg=True):
        ...
        for epoch in range(self.epochs):
            ...
            for step, (input_image, mask, bbox) in enumerate(progress_bar):
                ...
                loss = seg_loss(mask_predictions[0], mask) + seg_loss(mask_predictions[1], mask) + seg_loss(
                    mask_predictions[2], mask) + seg_loss(mask_predictions[3], mask) + seg_loss(mask_predictions[4],
                                                                                                mask)

                mask_predictions = (mask_predictions[0] + mask_predictions[1] + mask_predictions[2] + mask_predictions[
                    3] + mask_predictions[4])/5
...
if __name__ == '__main__':
    ...
    parser.add_argument('--task_name', type=str, default='test3')
    parser.add_argument(
