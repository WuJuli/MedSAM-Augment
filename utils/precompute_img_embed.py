...
parser.add_argument('-i', '--img_path', type=str, default='./data/abdominal_US/RUS_npz_50',
                    help='# and also Tr_Release_Part2 when part1 is done')
...
parser.add_argument('--checkpoint', type=str, default='./work_dir/SAM/sam_vit_b_01ec64.pth',
                    help='path to the pre-trained SAM model')
...
for name in tqdm(npz_files):
    ...
    gt = np.load(join(pre_img_path, name))['gts']  # (256, 256)
