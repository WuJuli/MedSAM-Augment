...
parser.add_argument('-gt', '--gt_path', type=str, default='./data/FLARE22Train/labels',
                    help='path to the ground truth', )
parser.add_argument('-o', '--npz_path', type=str, default='data/FLARE22Train/npz_files',
...
names = [name for name in names if
         not os.path.exists(join(args.npz_path, prefix + '_' + name.split('.nii.gz')[0] + '.npz'))]
...
def preprocess_ct(gt_path, nii_path, gt_name, image_name, label_id, image_size, sam_model, device):
    ...
    if np.sum(gt_data) > 1000:
        ...
        # print(image_data.shape, 'img_Data')
        ...
        for i in range(z_min, z_max):
            ...
            gt_slice_i = transform.resize(gt_slice_i, (image_size, image_size), order=0, preserve_range=True,
                                          mode='constant', anti_aliasing=False)
            if np.sum(gt_slice_i) > 100:
                img_slice_i = transform.resize(image_data_pre[i, :, :], (image_size, image_size), order=3,
...
for name in tqdm(train_names):
    ...
    gt_name = name
    imgs, gts, img_embeddings = preprocess_ct(args.gt_path, args.nii_path, gt_name, image_name, args.label_id,
    ...
    if len(imgs) > 1:
        ...
        np.savez_compressed(join(save_path_tr, prefix + '_' + gt_name.split('.nii.gz')[0] + '.npz'), imgs=imgs, gts=gts,
...
for name in tqdm(test_names):
    image_name = name.split('.nii.gz')[0] + args.img_name_suffix
    gt_name = name
    imgs, gts = preprocess_ct(args.gt_path, args.nii_path, gt_name, image_name, args.label_id, args.image_size,
                              sam_model=None, device=args.device)
    # %% save to npz file
    if len(imgs) > 1:
        imgs = np.stack(imgs, axis=0)  # (n, 256, 256, 3)
        gts = np.stack(gts, axis=0)  # (n, 256, 256)
