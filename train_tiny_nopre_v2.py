...
class TrainMedSam:
    ...
    def __init__(
            ...
            checkpoint: str = "work_dir/tiny_vit_sam/mobile_sam.pt",
    ...
    def train(self, model, train_loader: Iterable, logg=True):
        ...
        for epoch in range(self.epochs):
            ...
            for step, (img_batch, mask, bbox) in enumerate(progress_bar):
                batch_size = img_batch.shape[0]  # Get the batch size
                resize_img_tensors = []

                for i in range(batch_size):
                    img = img_batch[i].cpu()  # Move the tensor to CPU before converting to NumPy
                    ...
                    resize_img_tensors.append(input_image)

                input_images = torch.cat(resize_img_tensors, dim=0)
                # model input: (N, 3, 1024, 1024)
                # print(input_images.shape, "22222")

                assert input_images.shape == (batch_size, 3, model.image_encoder.img_size,
                ...
                with torch.inference_mode():
                    ...
                    image_embeddings = model.image_encoder(input_images)  # (B,256,64,64)
            ...
            if epoch_losses < best_loss:
                ...
                torch.save(
                    model.state_dict(),
                    join(self.save_path, 'Tiny_model_best.pth')
                )
...
if __name__ == '__main__':
    ...
    parser.add_argument('--task_name', type=str, default='batched_code')
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Path to Tiny vit checkpoint"
    )
